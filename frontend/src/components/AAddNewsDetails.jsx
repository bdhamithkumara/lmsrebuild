    ...
    <div>
      <div>
        <form>
          ...
          <button className="rounded-full bg-[#F1B81F] text-black p-2  w-[150px]" onClick={handleSubmit}>
            Save
          </button>
        </form>
      </div>
      <div>
        {allnews.map(mapgetall => (
          <Paper elevation={6} style={{margin:"10px",padding:"15px",textAlign:"left"}} key={mapgetall.id}>

          Id : {mapgetall.id} <br/>
          News Title : {mapgetall.newstitle} <br/>
          Hash Tags :  {mapgetall.hashtags} <br/>
          Content :  {mapgetall.content} <br/>
